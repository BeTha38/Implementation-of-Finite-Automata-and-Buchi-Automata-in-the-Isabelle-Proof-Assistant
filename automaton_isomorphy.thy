...
theorem expressive_power_dfa_nfa_main:
  assumes "infinite (UNIV :: 's set)"
  shows "{L . \<exists> (NFA_\<A> :: ('s, 'a) automaton) . auto_well_defined NFA_\<A> \<and> \<Sigma> = alphabet NFA_\<A> \<and> L = language_auto NFA_\<A>} = {L . \<exists> (DFA_\<A> :: ('s, 'a) automaton) . auto_well_defined DFA_\<A> \<and> DFA_property DFA_\<A> \<and> \<Sigma> = alphabet DFA_\<A> \<and> L = language_auto DFA_\<A>}"
