...
corollary same_finite_language_different_omega_language:
  assumes "infinite (UNIV :: 's set)" "infinite (UNIV :: 'a set)"
  shows "\<exists> (\<A>1 :: ('s, 'a) automaton) (\<A>2 :: ('s, 'a) automaton) . auto_well_defined \<A>1 \<and> auto_well_defined \<A>2 \<and> alphabet \<A>1 = alphabet \<A>2 \<and> language_auto \<A>1 = language_auto \<A>2 \<and> omega_language_auto \<A>1 \<noteq> omega_language_auto \<A>2"
proof -
  obtain \<A>1 :: "('s, 'a) automaton" where A1: "auto_well_defined \<A>1 \<and> (\<nexists> DBA :: ('s, 'a) automaton . auto_well_defined DBA \<and> DFA_property DBA \<and> alphabet DBA = alphabet \<A>1 \<and> omega_language_auto DBA = omega_language_auto \<A>1)" using NBA_to_DBA assms by blast
