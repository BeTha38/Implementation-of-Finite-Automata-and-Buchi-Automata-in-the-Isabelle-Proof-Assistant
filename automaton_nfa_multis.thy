...
text \<open>Author: Benno Thalmann, last update: 30.10.2025, Addition to masterarbeit_benno_thalmann\<close>
...
theorem expressive_power_multi_nfa_main:
  ...
  shows "{L . \<exists> (NFA_\<A> :: ('s, 'a) automaton) . auto_well_defined NFA_\<A> \<and> \<Sigma> = alphabet NFA_\<A> \<and> L = language_auto NFA_\<A>} = {L . \<exists> (NFA_\<A>_multi :: ('s, 'a) nfa_multi) . auto_well_defined_multi NFA_\<A>_multi \<and> \<Sigma> = alphabet_multi NFA_\<A>_multi \<and> L = language_auto_multi NFA_\<A>_multi}"
...
corollary union_main_better_nat:
  assumes "auto_well_defined (\<A>1 :: (nat, 'a) automaton)" "auto_well_defined (\<A>2 :: (nat, 'a) automaton)"
  shows "\<exists> union_\<A> :: (nat, 'a) automaton . auto_well_defined union_\<A> \<and> alphabet union_\<A> = alphabet \<A>1 \<union> alphabet \<A>2 \<and> language_auto union_\<A> = language_auto \<A>1 \<union> language_auto \<A>2"
  using existence_of_union_same_type assms infinite_UNIV_nat by blast
