...
corollary omega_reg_languages_well_def:
  assumes "L \<in> omega_regular_languages \<Sigma>"
  shows "omega_language_well_defined L \<Sigma>"
  using assms well_def_word_omega_reg_languages unfolding omega_language_well_defined_def by blast
...
definition omega_word_pow2_ab :: "'a symbol \<Rightarrow> 'a symbol \<Rightarrow> nat \<Rightarrow> 'a symbol" where "omega_word_pow2_ab a b n = (if (\<exists> k . n = 2^k) then a else b)"
