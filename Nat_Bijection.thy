...
text \<open>Author: Benno Thalmann, last update: 1.5.2026, Addition to automaton_isomorphy and masterarbeit_benno_thalmann\<close>
subsection \<open>Type \<^typ>\<open>nat + unit\<close>\<close>
definition sum_unit_encode :: "nat + unit \<Rightarrow> nat"
  where "sum_unit_encode x = (case x of Inl a \<Rightarrow> Suc a | Inr() \<Rightarrow> 0)"

definition sum_unit_decode :: "nat \<Rightarrow> nat + unit"
  where "sum_unit_decode n = (if n = 0 then Inr() else Inl(n - 1))"

lemma sum_unit_encode_inverse [simp]: "sum_unit_decode (sum_unit_encode x) = x"
  unfolding sum_unit_decode_def sum_unit_encode_def apply(induction x) apply simp by (simp add: case_unit_Unity)

lemma sum_unit_decode_inverse [simp]: "sum_unit_encode (sum_unit_decode n) = n"
  unfolding sum_unit_decode_def sum_unit_encode_def by (simp add: case_unit_Unity)

lemma inj_sum_unit_encode: "inj_on sum_unit_encode A"
  by (rule inj_on_inverseI) (rule sum_unit_encode_inverse)

lemma inj_sum_unit_decode: "inj_on sum_unit_decode A"
  by (rule inj_on_inverseI) (rule sum_unit_decode_inverse)

lemma surj_sum_unit_encode: "surj sum_unit_encode"
  by (rule surjI) (rule sum_unit_decode_inverse)

lemma surj_sum_unit_decode: "surj sum_unit_decode"
  by (rule surjI) (rule sum_unit_encode_inverse)

lemma bij_sum_unit_encode: "bij sum_unit_encode"
  by (rule bijI [OF inj_sum_unit_encode surj_sum_unit_encode])

lemma bij_sum_unit_decode: "bij sum_unit_decode"
  by (rule bijI [OF inj_sum_unit_decode surj_sum_unit_decode])

lemma sum_unit_encode_eq: "sum_unit_encode x = sum_unit_encode y \<longleftrightarrow> x = y"
  by (rule inj_sum_unit_encode [THEN inj_eq])

lemma sum_unit_decode_eq: "sum_unit_decode x = sum_unit_decode y \<longleftrightarrow> x = y"
  by (rule inj_sum_unit_decode [THEN inj_eq])


subsection \<open>Type \<^typ>\<open>nat \<times> nat + unit\<close>\<close>
definition cross_sum_unit_encode :: "nat \<times> nat + unit \<Rightarrow> nat"
  where "cross_sum_unit_encode x = (case x of Inl a \<Rightarrow> Suc (prod_encode a) | Inr() \<Rightarrow> 0)"

definition cross_sum_unit_decode :: "nat \<Rightarrow> nat \<times> nat + unit"
  where "cross_sum_unit_decode n = (if n = 0 then Inr() else Inl(prod_decode (n - 1)))"

lemma cross_sum_unit_encode_inverse [simp]: "cross_sum_unit_decode (cross_sum_unit_encode x) = x"
  unfolding cross_sum_unit_decode_def cross_sum_unit_encode_def apply(induction x) apply simp by (simp add: case_unit_Unity)

lemma cross_sum_unit_decode_inverse [simp]: "cross_sum_unit_encode (cross_sum_unit_decode n) = n"
  unfolding cross_sum_unit_decode_def cross_sum_unit_encode_def by (simp add: case_unit_Unity)

lemma inj_cross_sum_unit_encode: "inj_on cross_sum_unit_encode A"
  by (rule inj_on_inverseI) (rule cross_sum_unit_encode_inverse)

lemma inj_cross_sum_unit_decode: "inj_on cross_sum_unit_decode A"
  by (rule inj_on_inverseI) (rule cross_sum_unit_decode_inverse)

lemma surj_cross_sum_unit_encode: "surj cross_sum_unit_encode"
  by (rule surjI) (rule cross_sum_unit_decode_inverse)

lemma surj_cross_sum_unit_decode: "surj cross_sum_unit_decode"
  by (rule surjI) (rule cross_sum_unit_encode_inverse)

lemma bij_cross_sum_unit_encode: "bij cross_sum_unit_encode"
  by (rule bijI [OF inj_cross_sum_unit_encode surj_cross_sum_unit_encode])

lemma bij_cross_sum_unit_decode: "bij cross_sum_unit_decode"
  by (rule bijI [OF inj_cross_sum_unit_decode surj_cross_sum_unit_decode])

lemma cross_sum_unit_encode_eq: "cross_sum_unit_encode x = cross_sum_unit_encode y \<longleftrightarrow> x = y"
  by (rule inj_cross_sum_unit_encode [THEN inj_eq])

lemma cross_sum_unit_decode_eq: "cross_sum_unit_decode x = cross_sum_unit_decode y \<longleftrightarrow> x = y"
  by (rule inj_cross_sum_unit_decode [THEN inj_eq])
