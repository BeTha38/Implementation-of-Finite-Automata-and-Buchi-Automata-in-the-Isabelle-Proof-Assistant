...
imports Main automaton_minimization omega_regular_languages

begin

text \<open>Author: Benno Thalmann, last update: 14.5.2026, Addition to masterarbeit_benno_thalmann\<close>
...
text \<open>Examples: An omega_word in the eilenberg_limit, but not in the language, thus for this NBA those two omega_languages are not the same\<close> 
definition omega_word_alternating :: "nat \<Rightarrow> nat" where "omega_word_alternating n = (if even n then 1 else 0)"

lemma pre_omega_word_altering_in_lang:
  assumes "odd n"
  shows "pre_omega_word omega_word_alternating n \<in> language_auto \<A>_NBA"
proof -
  have n: "n > 0" using assms by presburger
  hence init: "run0 n ! 0 = initial_state \<A>_NBA \<and> initial_state \<A>_NBA \<in> states \<A>_NBA" unfolding \<A>_NBA_def by(induction n) auto
  have len: "length (run0 n) = Suc n" by(induction n) auto
  hence length: "length (run0 n) = length (pre_omega_word omega_word_alternating n) + 1" using pre_omega_word_length Suc_eq_plus1 by metis
  have word_well_def: "omega_word_well_defined (omega_word_alternating) (alphabet \<A>_NBA)" unfolding omega_word_well_defined_def omega_word_alternating_def \<A>_NBA_def by auto
  {
    fix i assume "i < length (run0 n) - 1"
    hence i: "i < n" using len by auto
    hence run0: "(run0 n) ! i = 0" using properties_run0 by simp
    have "(pre_omega_word omega_word_alternating n) ! i \<in> alphabet \<A>_NBA" using i word_well_def pre_omega_word_nth unfolding omega_word_well_defined_def by metis
    hence word: "(pre_omega_word omega_word_alternating n) ! i = 0 \<or> (pre_omega_word omega_word_alternating n) ! i = 1" unfolding \<A>_NBA_def by auto
    consider (case1) "i < n - 1" | (case2) "i = n - 1" using i by linarith 
    hence "((run0 n) ! i, (pre_omega_word omega_word_alternating n) ! i, (run0 n) ! (i + 1)) \<in> transitions \<A>_NBA" 
    proof cases
      case case1
      hence "(run0 n) ! (i + 1) = 0" using properties_run0 by blast
      thus ?thesis using run0 word unfolding \<A>_NBA_def by auto
    next
      case case2
      hence i1: "i + 1 = n" using n by auto
      hence "even i \<and> (pre_omega_word omega_word_alternating n) ! i = omega_word_alternating i" using assms pre_omega_word_nth i by auto
      hence word0: "(pre_omega_word omega_word_alternating n) ! i = 1" unfolding omega_word_alternating_def by argo
      have "run0 n \<noteq> [] \<and> length (run0 n) = n + 1" using properties_run0 by auto
      hence "(run0 n) ! (i + 1) = last (run0 n)" using i1 list_properties_not_empty by fastforce
      hence "(run0 n) ! (i + 1) = 1" using properties_run0 by metis
      thus ?thesis using word0 run0 unfolding \<A>_NBA_def by auto
    qed
  }
  hence "\<forall> i < length (run0 n) - 1 . ((run0 n) ! i, (pre_omega_word omega_word_alternating n) ! i, (run0 n) ! (i + 1)) \<in> transitions \<A>_NBA" by blast
  hence run_well_def: "pseudo_run_well_defined (run0 n) \<A>_NBA (initial_state \<A>_NBA) (pre_omega_word omega_word_alternating n)" using init length unfolding pseudo_run_well_defined_def by blast
  have "last (run0 n) \<in> accepting_states \<A>_NBA" using n properties_run0 unfolding \<A>_NBA_def by auto
  hence "run_accepting (run0 n) \<A>_NBA (pre_omega_word omega_word_alternating n)" using run_well_def unfolding run_well_defined_def run_accepting_def by blast
  thus ?thesis unfolding language_auto_def by blast
qed

proposition omega_word_alt_in_eilenberg: "omega_word_alternating \<in> eilenberg_limit (language_auto \<A>_NBA)"
proof - 
  {
    fix n::nat
    consider (case1) "odd n" | (case2) "even n" by blast
    hence "\<exists> N > n . pre_omega_word omega_word_alternating N \<in> language_auto \<A>_NBA"
    proof cases
      case case1
      obtain N where "N = n + 2" by blast
      hence "odd N \<and> N > n" using case1 by simp
      thus ?thesis using pre_omega_word_altering_in_lang by blast
    next
      case case2
      obtain N where "N = n + 1" by blast
      hence "odd N \<and> N > n" using case2 by simp
      thus ?thesis using pre_omega_word_altering_in_lang by blast
    qed
  }
  thus ?thesis using equi_criterion_eilenberg by blast
qed

proposition omega_word_alt_not_in_lang: "omega_word_alternating \<notin> omega_language_auto \<A>_NBA"
proof(rule ccontr)
  assume "\<not> omega_word_alternating \<notin> omega_language_auto \<A>_NBA"
  then obtain omega_run where "omega_run_accepting omega_run \<A>_NBA omega_word_alternating" unfolding omega_language_auto_def by fast
  hence props: "omega_pseudo_run_well_defined omega_run \<A>_NBA (initial_state \<A>_NBA) omega_word_alternating \<and> (\<forall> n . \<exists> N > n . omega_run N \<in> accepting_states \<A>_NBA)" unfolding omega_run_accepting_def omega_run_well_defined_def by blast
  then obtain N where "omega_run N \<in> accepting_states \<A>_NBA" by blast
  hence N: "omega_run N = 1" unfolding \<A>_NBA_def by auto
  have trans: "(omega_run N, omega_word_alternating N, omega_run (N + 1)) \<in> transitions \<A>_NBA" using props unfolding omega_pseudo_run_well_defined_def by blast
  consider (case1) "omega_word_alternating N = 0" | (case2) "omega_word_alternating N = 1" unfolding omega_word_alternating_def by argo
  thus False
  proof cases
    case case1 thus ?thesis using N trans unfolding \<A>_NBA_def by force
  next
    case case2
    hence "omega_run (N + 1) = 1" using N trans unfolding \<A>_NBA_def by auto
    hence N1: "omega_run (N + 1) = 1 \<and> omega_word_alternating (N + 1) = 0" using case2 add.commute dvd_add_left_iff odd_one unfolding omega_word_alternating_def by metis
    have "(omega_run (N + 1), omega_word_alternating (N + 1), omega_run (N + 1 + 1)) \<in> transitions \<A>_NBA" using props unfolding omega_pseudo_run_well_defined_def by auto
    thus ?thesis using N1 unfolding \<A>_NBA_def by auto
  qed
qed

theorem not_the_same_omega_lang: "eilenberg_limit (language_auto \<A>_NBA) \<noteq> omega_language_auto \<A>_NBA"
  using omega_word_alt_in_eilenberg omega_word_alt_not_in_lang by blast

text \<open>Examples: The prefixes of omega_word_pow2 are not a regular language\<close> 
definition pow2_prefixes :: "nat language" where "pow2_prefixes = pre_omega_words (omega_word_pow2_ab 1 0)"

lemma pow2_prefixes_not_regular: "pow2_prefixes \<notin> regular_languages {0, 1}"
proof(rule ccontr)
  assume "\<not> pow2_prefixes \<notin> regular_languages {0, 1}"
  hence reg: "pow2_prefixes \<in> regular_languages {0, 1}" by blast
  then obtain n where n: "n > 0 \<and> (\<forall> word \<in> pow2_prefixes . length word \<ge> n \<longrightarrow> (\<exists> u v w . word = u @ v @ w \<and> length (u @ v) \<le> n \<and> length v \<ge> 1 \<and> (\<forall> i . u @ pump v i @ w \<in> pow2_prefixes)))" using pumping_lemma by force
  obtain k where k: "n < 2^k" using less_exp by blast
  define N :: nat where "N = 2^(k+1)"
  define word where "word = pre_omega_word (omega_word_pow2_ab (1 :: nat) 0) N"
  hence word_in: "word \<in> pow2_prefixes" unfolding pow2_prefixes_def pre_omega_words_def by blast
  have "length word \<ge> n" using k unfolding word_def N_def pre_omega_word_length by simp
  then obtain u v w where uvw: "word = u @ v @ w \<and> length (u @ v) \<le> n \<and> length v \<ge> 1 \<and> (\<forall> i . u @ pump v i @ w \<in> pow2_prefixes)" using n word_in by blast
  define d where "d = length v"
  hence d_pos: "d > 0" using uvw by auto
  have d_lt: "d < 2^k" using uvw k unfolding d_def by simp
  have "length u \<le> n" using uvw by simp
  hence u_before: "length u \<le> 2^k" using k by simp
  have "pump v 0 = []" by simp
  hence "u @ w \<in> pow2_prefixes" using uvw append.left_neutral by metis
  then obtain M where M: "u @ w = pre_omega_word (omega_word_pow2_ab 1 0) M" unfolding pow2_prefixes_def pre_omega_words_def by blast
  have len_word: "length word = N" unfolding word_def by (simp add: pre_omega_word_length)
  have "length (u @ w) = length word - d" using uvw unfolding d_def by auto
  hence len_M: "M = N - d" using M len_word by (metis pre_omega_word_length)
  have "2^k + d < 2^(k+1)" using d_lt by simp
  hence "2^k < 2^(k+1) - d" by simp
  hence pow_pos_in_M: "2^k < M" using len_M unfolding N_def by simp
  hence "2^k < length (u @ w)" using M by (metis pre_omega_word_length)
  hence 1: "(u @ w) ! (2^k) = w ! (2^k - length u)" using u_before by (simp add: nth_append)
  have word_decomp: "word = u @ v @ w" using uvw by blast
  have le_ud: "length u + d \<le> 2^k + d" using u_before by simp
  have lt_word: "2^k + d < length word" using pow_pos_in_M len_M len_word by simp
  hence nth1: "word ! (2^k + d) = (v @ w) ! ((2^k + d) - length u)" using word_decomp le_ud by (simp add: nth_append)
  have ge_d: "d \<le> (2^k + d) - length u" using u_before by simp
  have "(2^k + d) - length u < length (v @ w)" using word_decomp lt_word le_ud by auto
  hence nth2: "(v @ w) ! ((2^k + d) - length u) = w ! (((2^k + d) - length u) - d)" using ge_d unfolding d_def by (simp add: nth_append)
  have "((2^k + d) - length u) - d = 2^k - length u" using u_before by simp
  hence 2: "word ! (2^k + d) = w ! (2^k - length u)" using nth1 nth2 by simp
  have "2^k + d < N" using pow_pos_in_M len_M by simp
  hence "word ! (2^k + d) = (omega_word_pow2_ab 1 0) (2^k + d)" unfolding word_def by (simp add: pre_omega_word_nth)
  hence nth_pumped: "(u @ w) ! (2^k) = (omega_word_pow2_ab 1 0) (2^k + d)" using 1 2 by argo
  have "(u @ w) ! (2^k) = (omega_word_pow2_ab 1 0) (2^k)" using pow_pos_in_M  M by (simp add: pre_omega_word_nth)
  hence eq: "(omega_word_pow2_ab (1 :: nat) 0) (2^k) = (omega_word_pow2_ab 1 0) (2^k + d)" using nth_pumped by simp 
  have left: "(omega_word_pow2_ab (1 :: nat) 0) (2^k) = 1"  unfolding omega_word_pow2_ab_def by auto
  have "(omega_word_pow2_ab (1 :: nat) 0) (2^k + d) = 0" using pow2_gap_not_pow2 d_pos d_lt unfolding omega_word_pow2_ab_def by auto
  thus False using eq left by simp
qed
