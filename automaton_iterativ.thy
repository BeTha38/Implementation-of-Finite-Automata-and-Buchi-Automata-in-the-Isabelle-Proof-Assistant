...
lemma reachable_iter_inheritance_general: "s' \<in> reachable_states_iter n \<A> S \<and> (s', a, s) \<in> transitions \<A> \<Longrightarrow> s \<in> reachable_states_iter (Suc n) \<A> S"
...
lemma equi_sublist:
  assumes "run_well_defined run \<A> word" "i < length run \<and> j < length run \<and> i \<noteq> j \<and> run ! i = run ! j \<and> i < j" "j \<noteq> length word"
...
proof -
  ...
  hence sub_trans: "\<forall> m . m < length ?subrun2 + length ?subrun1 - 1 \<and> m \<ge> length ?subrun1 \<longrightarrow> (?subrun2 ! (m - length ?subrun1), ?subword2 ! (m + 1 - length ?subrun1), ?subrun2 ! (m + 1 - length ?subrun1)) \<in> transitions \<A>" by blast
  {
  ...
  hence "\<forall> k < length ?subrun - 1 . (?subrun ! k, ?subword ! k, ?subrun ! (k + 1)) \<in> transitions \<A>" by blast
  thus ?thesis using length by blast
qed

lemma subrun_initial_state:
  assumes "run_well_defined run \<A> word" "i < length run \<and> j < length run \<and> i \<noteq> j \<and> run ! i = run ! j \<and> i < j" 
...
lemma subrun_last_state:
  assumes "run_well_defined run \<A> word" "i < length run \<and> j < length run \<and> i \<noteq> j \<and> run ! i = run ! j \<and> i < j" "s = last run"
...
next
  ...
  next
    ...
    thus ?thesis using reachable_iter_inheritance_general by fast
  qed
qed
...
lemma eps_clos_inheritance_general: "s' \<in> epsilon_closure_iter n \<A> S \<and> (s', Inr(), s) \<in> transitions \<A> \<Longrightarrow> s \<in> epsilon_closure_iter (Suc n) \<A> S"
...
proof -
  ...
  hence sub_trans: "\<forall> m . m < length ?subrun2 + length ?subrun1 - 1 \<and> m \<ge> length ?subrun1 \<longrightarrow> (?subrun2 ! (m - length ?subrun1), Inr(), ?subrun2 ! (m + 1 - length ?subrun1)) \<in> transitions \<A>" by blast
  {
  ...
  hence "\<forall> k < length ?subrun - 1 . (?subrun ! k, Inr(), ?subrun ! (k + 1)) \<in> transitions \<A>" by blast
  thus ?thesis using length by blast
qed
...
next
  ...
  next
    ...
    thus ?thesis using eps_clos_inheritance_general by fast
  qed
qed
