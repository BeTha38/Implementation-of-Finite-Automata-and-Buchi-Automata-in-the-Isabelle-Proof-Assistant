...
lemma NFA_to_epsi_alphabet: "alphabet \<A> = original_alphabet (alphabet (NFA_to_epsi \<A>))"
...
lemma eps_run_well_defined_induction:
  assumes "eps_run_well_defined eps_run \<A> s" "(last eps_run, Inr(), s') \<in> transitions \<A>"
...
lemma no_last_transition_eps:
  assumes "eps_run_well_defined (eps_run @ [s']) \<A> s" "length (eps_run @ [s']) = 1"
  shows "s = s'"
...
proposition eps_run_well_defined_extension: "eps_run_well_defined (eps_run @ [s']) \<A> s \<and> length (eps_run @ [s']) > 1 \<longleftrightarrow> eps_run_well_defined eps_run \<A> s \<and> (last eps_run, Inr(), s') \<in> transitions \<A>"
  using eps_run_well_defined_snoc eps_run_well_defined_induction by metis
...
lemma transitivity_esp_connected:
  assumes "epsilon_connected s1 \<A> s2" "(s2, Inr(), s) \<in> transitions \<A>"
...
next
  ...
  next
    ...
    next
      ...
      hence well_def: "run_well_defined (run' @ [last run]) \<A> word \<and> (last run, a, s) \<in> transitions \<A>" using snoc prun_well_defined_extension_snoc unfolding run_well_defined_def by metis
...
next
  ...
  then obtain run' where run: "run = (run' @ [last run])" using append_butlast_last_id by metis
  hence run': "run_well_defined run' \<A> word" using prun_well_defined_extension_snoc well_def_run unfolding run_well_defined_def by metis 
...
next
  ...
  hence len: "length run = length (word @ [a]) + 1 \<and> run \<noteq> []" unfolding run_well_defined_def pseudo_run_well_defined_def by force
  then obtain run' where run: "run = (run' @ [last run])" using append_butlast_last_id by metis
  hence run': "run_well_defined run' \<A> word \<and> (last run', a, last run) \<in> transitions \<A>" using prun_well_defined_extension_snoc well_def_run unfolding run_well_defined_def by metis
...
lemma normalize_epsilon_run_i_step:
  assumes "run_well_defined run' \<A> word' \<and> last run' = last run \<and> evaluate_eps_word word' = word \<and> eps_run ! 0 = last run \<and> length eps_run > 0 \<and> (\<forall> i < length eps_run - 1 . (eps_run ! i, Inr(), eps_run ! (i + 1)) \<in> transitions \<A>)"
...
proof(induction word arbitrary: run rule: rev_induct)
  let ?\<A> = "normalize_epsilon_nfa \<A>"
  case Nil
  hence "length run = 1 \<and> run ! 0 = initial_state ?\<A>" unfolding run_well_defined_def pseudo_run_well_defined_def by force
...
next
  ...
  then obtain run' where "run = (run' @ [last run])" using append_butlast_last_id by metis
  hence "run_well_defined run' ?\<A> word \<and> (last run', a, last run) \<in> transitions ?\<A>" using prun_well_defined_extension_snoc snoc unfolding run_well_defined_def by metis
  ...
  have "\<forall> i < length (run_eps @ (tl eps_run)) - 1 . ((run_eps @ (tl eps_run)) ! i, ((word_eps @ (only_eps_word (length (tl eps_run))))) ! i, (run_eps @ (tl eps_run)) ! (i + 1)) \<in> transitions \<A>" using eps_run run_word_eps normalize_epsilon_run_i_step by blast
...
proof - 
  ...
  hence "(\<forall> i < length (run_transformation_eps run) - 1 . ((run_transformation_eps run) ! i, (word_transformation_eps word) ! i, (run_transformation_eps run) ! (i + 1)) \<in> transitions (normalize_nfa_eps \<A>))" using length by argo
...
lemma nfa_multi_run_eps_eval_tl:
  assumes "run_well_defined run (normalize_nfa_eps \<A>) word"
...
proposition run_transformation_eps_right_base:
  assumes "run_well_defined run (normalize_nfa_eps \<A>) []" 
...
proof(induction word arbitrary: run a rule: rev_induct)
  case Nil
  let ?\<A> = "normalize_nfa_eps \<A>"
  have init: "run ! 0 = Inr()" using Nil unfolding run_well_defined_def pseudo_run_well_defined_def normalize_nfa_eps_def by force
...
next
  ...
  hence "run \<noteq> []" unfolding run_well_defined_def pseudo_run_well_defined_def by force
  then obtain run' where "run = run' @ [last run]" using append_butlast_last_id by metis
  hence trans: "run_well_defined run' ?\<A> (word @ [a']) \<and> (last run', a, last run) \<in> transitions ?\<A>" using prun_well_defined_extension_snoc snoc unfolding run_well_defined_def by metis
  ...
  have "Inr() \<notin> set (tl ((word @ [a']) @ [a]))" using snoc nfa_multi_word_eps_eval_tl by blast
  ...
  hence existence: "run_well_defined_multi (run'' @ [s']) \<A> (word'' @ [b]) \<and> Inl s' = last run" using s' run_well_defined_extension_snoc_multi by fast
...
proof -
  let ?\<A> = "normalize_nfa_eps \<A>"
  {
    ...
    hence "word \<noteq> []" using run_transformation_eps_right_base by auto
...
lemma image_composition_language: "language_auto_multi \<A> = image evaluate_eps_word (image word_transformation_eps (language_auto_multi \<A>))"
proof - 
  {
