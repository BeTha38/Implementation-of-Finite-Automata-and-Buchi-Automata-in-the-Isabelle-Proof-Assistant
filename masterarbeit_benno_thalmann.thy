...
text \<open>Last updated: 02.10.2025\<close>
...
text \<open>Predicate if word is over an alphabet of an automaton\<close>
...
lemma prun_well_defined_induction_snoc:
  assumes "auto_well_defined \<A>" "pseudo_run_well_defined prun \<A> s' word" "(last prun, a, s) \<in> transitions \<A>"
...
lemma last_transition:
  assumes "auto_well_defined \<A>" "pseudo_run_well_defined (prun @ [s]) \<A> s' (word @ [a])"
...
proposition prun_well_defined_extension_snoc:
  assumes "auto_well_defined \<A>"
  shows "pseudo_run_well_defined (prun @ [s]) \<A> s' (word @ [a]) \<longleftrightarrow> pseudo_run_well_defined prun \<A> s' word \<and> (last prun, a, s) \<in> transitions \<A>"
  using assms prun_well_defined_snoc prun_well_defined_induction_snoc by metis
...
next
  case (snoc a word)
  {
    ...
    hence well_def: "pseudo_run_well_defined (prun @ [s']) \<A> s (word @ [a])" using assms prun_well_defined_extension_snoc by fast
    {
      ...
      hence "pseudo_run_well_defined prunA \<A> s word \<and> pseudo_run_well_defined prunB \<A> s word" using prun_well_defined_extension_snoc assm assms by fast
      hence equi: "prunA = prunB" using assm existence by auto
      have "(last prunA, a, sA) \<in> transitions \<A> \<and> (last prunB, a, sB) \<in> transitions \<A>" using assms assm pruns prun_well_defined_extension_snoc by fast
...
corollary exists_only_one_run_for_DFA:
  assumes "auto_well_defined \<A>" "DFA_property \<A>"
  shows "word_well_defined word (alphabet \<A>) \<Longrightarrow> \<exists>! run . run_well_defined run \<A> word"
...
proposition language_subset_of_sigma_star:
...
theorem comp_comp_language: 
  ...
  using assms comp_language_symmetry comp_language_well_defined unfolding comp_language_def language_well_defined_def by metis
...
proof(rule ccontr)
  assume "L1 \<noteq> L2"
  then obtain word where word: "(word \<in> L1 \<and> word \<notin> L2) \<or> (word \<in> L2 \<and> word \<notin> L1)" unfolding language_auto_def by blast
...
lemma comp_automaton_alphabet:
  assumes "auto_well_defined \<A>"
  shows "alphabet \<A> = alphabet (comp_automaton \<A>)"
...
corollary comp_comp_language_auto:
  ...
  using comp_automaton_language_correctness comp_comp_language  assms comp_automaton_alphabet automata_language_are_well_defined by metis
...
lemma powerset_automaton_DFA_uniqueness:
  assumes "auto_well_defined \<A>" "S2 \<in> states (powerset_automaton \<A>)" "S2' \<in> states (powerset_automaton \<A>)" "(S1, a, S2) \<in> transitions (powerset_automaton \<A>)" "(S1, a, S2') \<in> transitions (powerset_automaton \<A>)"
  shows "S2 = S2'"
...
next
  ...
  have "pseudo_run_well_defined (pRun @ [S2]) ?\<A> S (word @ [a]) \<and> s \<in> S \<and> S \<in> states ?\<A>" using prun_well_defined_extension_snoc well_def_auto S2 pRun by fast
...
proof(induction word arbitrary: run rule: rev_induct)
  ...
  case Nil
  hence "run = [initial_state \<A>]" using assms list_with_one_element unfolding run_well_defined_def pseudo_run_well_defined_def by fastforce
...
next
  ...
  hence "run_well_defined run' \<A> word \<and> (last run', a, last run) \<in> transitions \<A>" using prun_well_defined_extension_snoc well_def assms unfolding run_well_defined_def by metis  
...
lemma powerset_transition_implies_transition:
  assumes "auto_well_defined \<A>" "(S1, a, S2) \<in> transitions (powerset_automaton \<A>)" "S2 \<noteq> {}"
  shows "S1 \<noteq> {} \<and> (\<forall> s2 \<in> S2 . \<exists> s1 \<in> S1 . (s1, a, s2) \<in> transitions \<A>)" 
  using assms unfolding powerset_automaton_def powerset_transitions_def by auto

lemma powerset_prun_right:
  assumes "auto_well_defined \<A>" 
  shows "pseudo_run_well_defined pRun (powerset_automaton \<A>) S word \<and> last pRun \<noteq> {} \<Longrightarrow> \<forall> s \<in> last pRun . \<exists> prun . \<exists> s' \<in> S . pseudo_run_well_defined prun \<A> s' word \<and> last prun = s"
proof(induction word arbitrary: pRun rule: rev_induct)
  let ?\<A> = "powerset_automaton \<A>"
  case Nil
  hence "pRun = [S] \<and> S \<in> states ?\<A>" using assms list_with_one_element unfolding pseudo_run_well_defined_def by fastforce
...
next
  let ?\<A> = "powerset_automaton \<A>"
  have well_def_auto: "auto_well_defined ?\<A>" using assms well_def_powerset_automaton by auto
  case (snoc a word)
  hence well_def: "pseudo_run_well_defined pRun ?\<A> S (word @ [a]) \<and> last pRun \<noteq> {}" using assms well_def_auto by blast
  ...
  hence not_empty: "last pRun' \<noteq> {}" using assms powerset_transition_implies_transition length by metis
  have "pseudo_run_well_defined pRun' ?\<A> S word" using pRun' prun_well_defined_extension_snoc well_def assms well_def_auto by metis
  ...
  have trans_in_trans: "\<forall> s' \<in> last pRun . \<exists> s'' \<in> last pRun' . (s'', a, s') \<in> transitions \<A>" using trans powerset_transition_implies_transition assms by fast
  {
    ...
    hence "pseudo_run_well_defined (prun @ [s']) \<A> s''' (word @ [a]) \<and> s''' \<in> S \<and> s' = last (prun @ [s'])" using assms s'' prun_well_defined_extension_snoc snoc by fastforce 
...
corollary powerset_run_right:
  assumes "auto_well_defined \<A>" 
  shows "run_well_defined Run (powerset_automaton \<A>) word \<and> last Run \<noteq> {} \<Longrightarrow> \<forall> s \<in> last Run . \<exists> run . run_well_defined run \<A> word \<and> last run = s"
  using assms powerset_prun_right unfolding run_well_defined_def powerset_automaton_def by force

lemma powerset_language_right:
  assumes "auto_well_defined \<A>"
  shows "language_auto (powerset_automaton \<A>) \<subseteq> language_auto \<A>"
proof -
  let ?\<A> = "powerset_automaton \<A>"
  {
    fix word assume "word \<in> language_auto ?\<A>"
    ...
    then obtain run where "run_well_defined run \<A> word \<and> last run \<in> accepting_states \<A>" using existence Run assms powerset_run_right by metis
...
lemma product_run_accepting_inter: 
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2" "run_accepting run1 \<A>1 word" "run_accepting run2 \<A>2 word"
...
theorem inter_implication_single_to_product:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2" "word \<in> language_auto \<A>1" "word \<in> language_auto \<A>2"
...
lemma single_run_accepting_inter:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2" "run_accepting run (intersection_automaton \<A>1 \<A>2) word"
...
theorem inter_implication_product_to_single:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2" "word \<in> language_auto (intersection_automaton \<A>1 \<A>2)"
...
theorem closed_under_intersection:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2"
  shows "word \<in> language_auto (intersection_automaton \<A>1 \<A>2) \<longleftrightarrow> word \<in> language_auto \<A>1 \<and> word \<in> language_auto \<A>2"
  using inter_implication_single_to_product inter_implication_product_to_single assms by metis

text \<open>Main result for intersection\<close>
theorem language_intersection_correctness:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2"
  shows "language_auto (intersection_automaton \<A>1 \<A>2) = language_auto \<A>1 \<inter> language_auto \<A>2"
  using assms closed_under_intersection by auto
...
lemma product_run_accepting_union_\<A>1:
  assumes "auto_well_defined \<A>1" "DFA_property \<A>1" "auto_well_defined \<A>2" "DFA_property \<A>2" "alphabet \<A>1 = alphabet \<A>2" "run_accepting run1 \<A>1 word"
...
lemma product_run_accepting_union_\<A>2:
  assumes "auto_well_defined \<A>1" "DFA_property \<A>1" "auto_well_defined \<A>2" "DFA_property \<A>2" "alphabet \<A>1 = alphabet \<A>2" "run_accepting run2 \<A>2 word"
...
lemma union_implication_\<A>1_to_product:
  assumes "auto_well_defined \<A>1" "DFA_property \<A>1" "auto_well_defined \<A>2" "DFA_property \<A>2" "alphabet \<A>1 = alphabet \<A>2" "word \<in> language_auto \<A>1"
...
lemma union_implication_\<A>2_to_product:
  assumes "auto_well_defined \<A>1" "DFA_property \<A>1" "auto_well_defined \<A>2" "DFA_property \<A>2" "alphabet \<A>1 = alphabet \<A>2" "word \<in> language_auto \<A>2"
  shows "word \<in> language_auto (union_automaton \<A>1 \<A>2)"
...
lemma single_run_accepting_union:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2" "run_accepting run (union_automaton \<A>1 \<A>2) word"
  shows "run_accepting (map fst run) \<A>1 word \<or> run_accepting (map snd run) \<A>2 word "
proof -
  let ?\<A> = "(union_automaton \<A>1 \<A>2)"
  let ?run1 = "(map fst run)"
  let ?run2 = "(map snd run)"
  have "run_well_defined run ?\<A> word \<and> auto_well_defined ?\<A>" using assms well_def_inter_union_automata unfolding run_accepting_def by auto
  hence "length run = length word + 1 \<and> run \<noteq> []" unfolding run_well_defined_def pseudo_run_well_defined_def by force
  hence "run \<noteq> [] \<and> last run = run ! (length run - 1)" using list_properties_not_empty by metis
  hence "last run = (last ?run1, last ?run2)" using assms by (simp add: last_map)
...
theorem union_implication_product_to_single:
  assumes "auto_well_defined \<A>1" "auto_well_defined \<A>2" "alphabet \<A>1 = alphabet \<A>2" "word \<in> language_auto (union_automaton \<A>1 \<A>2)"
...
proof -
  obtain \<A>3 :: "('s states, 'a) automaton" where A3: "auto_well_defined \<A>3 \<and> alphabet \<A>1 = alphabet \<A>3 \<and> DFA_property \<A>3 \<and> language_auto \<A>3 = language_auto \<A>1" using assms NFA_to_DFA by metis
  ...
  obtain \<A>6 :: "('t states, 'a) automaton" where "auto_well_defined \<A>6 \<and> alphabet \<A>2 = alphabet \<A>6 \<and> DFA_property \<A>6 \<and> language_auto \<A>6 = language_auto \<A>2" using NFA_to_DFA assms by metis
