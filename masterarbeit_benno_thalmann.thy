...
text \<open>Author: Benno Thalmann, Date: 2.1.2025, Master Thesis in Comptuer Science, University of Fribourg\<close>
text \<open>Last updated: 14.5.2026\<close>
...
proposition maximal_prefix_run:
  assumes "auto_well_defined \<A>" "\<nexists> run . run_well_defined run \<A> word"
  shows "\<exists> i < length word . (\<exists> run . run_well_defined run \<A> (take i word)) \<and> (\<forall> run . run_well_defined run \<A> (take i word) \<longrightarrow> (\<nexists> s . s \<in> states \<A> \<and> (last run, word ! i, s) \<in> transitions \<A>))"
proof -
  have empty_word: "run_well_defined [initial_state \<A>] \<A> []" using assms unfolding run_well_defined_def pseudo_run_well_defined_def auto_well_defined_def by auto
  define I where "I = {i . i \<le> length word \<and> (\<exists> run. run_well_defined run \<A> (take i word))}"
  hence zero_in_I: "0 \<in> I" using empty_word by auto
  have "finite I" unfolding I_def by fast
  then obtain i where i: "i \<in> I \<and> (\<forall> j \<in> I . j \<le> i)" using zero_in_I finite_has_maximal by fastforce
  hence i_le: "i \<le> length word" unfolding I_def by blast
  obtain run where run: "run_well_defined run \<A> (take i word)" using i unfolding I_def by blast
  have i_less: "i < length word"
  proof (rule ccontr)
    assume "\<not> i < length word"
    hence "i = length word" using i_le by auto
    hence "(\<exists> run . run_well_defined run \<A> word)" using run by force
    thus False using assms by auto
  qed
  {
    fix r assume assm: "run_well_defined r \<A> (take i word)"
    have "\<nexists> s .  s \<in> states \<A> \<and> (last r, word ! i, s) \<in> transitions \<A>"
    proof(rule ccontr)
      assume "\<not> (\<nexists> s .  s \<in> states \<A> \<and> (last r, word ! i, s) \<in> transitions \<A>)"
      hence "\<exists> s \<in> states \<A> . (last r, word ! i, s) \<in> transitions \<A>" by blast
      then obtain s where s: "s \<in> states \<A> \<and> (last r, word ! i, s) \<in> transitions \<A>" by blast
      hence well_def: "run_well_defined (r @ [s]) \<A> ((take i word) @ [word ! i])" using assm prun_well_defined_extension_snoc unfolding run_well_defined_def by fast
      have "(take i word) @ [word ! i] = take (Suc i) word" by (simp add: i_less take_Suc_conv_app_nth)
      hence "run_well_defined (r @ [s]) \<A> (take (Suc i) word)" using well_def by argo
      hence "Suc i \<in> I" using i_less unfolding I_def by force
      thus False using i by auto
    qed
  }
  hence "\<forall> run . run_well_defined run \<A> (take i word) \<longrightarrow> (\<nexists> s . s \<in> states \<A> \<and> (last run, word ! i, s) \<in> transitions \<A>)" by blast
  thus ?thesis using run i_less by blast
qed
...
proposition comp_language_characteristic:
  ...
  using assms language_well_def_iff unfolding comp_language_def by auto
...
definition product_automaton :: "('s, 'a) automaton \<Rightarrow> ('t , 'a) automaton \<Rightarrow> ('s \<times> 't) states \<Rightarrow> (('s \<times> 't), 'a) automaton" where
  "product_automaton \<A>1 \<A>2 F = Automaton
