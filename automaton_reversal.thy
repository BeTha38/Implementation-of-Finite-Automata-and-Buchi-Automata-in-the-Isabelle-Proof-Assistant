...
definition rev_language :: "'a language \<Rightarrow> 'a language" where "rev_language L = {rev word | word . word \<in> L}"
...
corollary expressive_power_rev_main_cor:
  ...
  shows "L \<in> {L . \<exists> (NFA_\<A> :: ('s, 'a) automaton) . auto_well_defined NFA_\<A> \<and> \<Sigma> = alphabet NFA_\<A> \<and> L = language_auto NFA_\<A>}"
proof - 
