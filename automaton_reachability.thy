...
theorem inheritance_reachable_state:
  assumes "reachable_state s1 \<A>" "(s1, a, s2) \<in> transitions \<A>"
...
lemma prun_reachable_automaton_left:
  ...
  using assms reachable_states_are_states unfolding pseudo_run_well_defined_def reachable_automaton_def reachable_states_def by auto
...
next
  ...
  hence "pseudo_run_well_defined prun' \<A> s word \<and> (last prun', a, last prun) \<in> transitions \<A>" using snoc prun_well_defined_extension_snoc by metis 
  ...
  hence "reachable_state (last prun') \<A> \<and> reachable_state (last prun) \<A>" using props inheritance_reachable_state by fast  
  hence "(last prun', a, last prun) \<in> transitions (reachable_automaton \<A>)" using props unfolding reachable_automaton_def by auto
  thus ?case using prun props prun_well_defined_extension_snoc by metis
qed
...
proof -
  let ?\<A> = "reachable_automaton \<A>"
  {
    ...
    hence "\<exists>! s2 \<in> states \<A> . (s1, a, s2) \<in> transitions \<A> \<and> s2 \<in> reachable_states \<A>" using reach inheritance_reachable_state unfolding reachable_states_def by fast
