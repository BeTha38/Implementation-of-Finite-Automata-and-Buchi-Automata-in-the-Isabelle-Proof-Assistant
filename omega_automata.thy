...
text \<open>Author: Benno Thalmann, last update: 13.5.2026, Addition to masterarbeit_benno_thalmann\<close>
...
lemma pre_omega_word_Suc: "pre_omega_word x (Suc n) = pre_omega_word x n @ [x n]"
  unfolding pre_omega_word_def by auto
...
lemma pre_omega_run_last: "last (pre_omega_run omega_run i) = omega_run i"
proof -
  have length: "length (pre_omega_run omega_run i) = Suc i" using pre_omega_run_length by blast
  hence "pre_omega_run omega_run i \<noteq> []" by force
  hence "last (pre_omega_run omega_run i) = (pre_omega_run omega_run i) ! (length (pre_omega_run omega_run i) - 1)" using list_properties_not_empty by metis
  thus ?thesis using pre_omega_run_nth length by auto
qed

lemma pre_omega_run_Suc: "pre_omega_run r (Suc n) = pre_omega_run r n @ [r (Suc n)]"
  unfolding pre_omega_run_def by auto
...
text \<open>Now we can prove some connection between empty languages and omega_languages\<close>
lemma accepted_omega_word_implies_infinite_finite_language:
  assumes "omega_word \<in> omega_language_auto \<A>"
  shows "infinite (language_auto \<A>)"
proof -
  obtain omega_run where run: "omega_run_accepting omega_run \<A> omega_word" using assms unfolding omega_language_auto_def by blast
  hence run_well: "omega_pseudo_run_well_defined omega_run \<A> (initial_state \<A>) omega_word" unfolding omega_run_accepting_def omega_run_well_defined_def by blast
  have acc_inf: "\<forall> n . \<exists> N > n . omega_run N \<in> accepting_states \<A>" using run unfolding omega_run_accepting_def by blast
  {
    fix n
    obtain N where N: "N > n \<and> omega_run N \<in> accepting_states \<A>" using acc_inf by blast
    have "pseudo_run_well_defined (pre_omega_run omega_run N) \<A> (initial_state \<A>) (pre_omega_word omega_word N) \<and> last (pre_omega_run omega_run N) = omega_run N" using omega_run_implies_pre_run_well_def run_well by metis
    hence "run_accepting (pre_omega_run omega_run N) \<A> (pre_omega_word omega_word N)" using N unfolding run_accepting_def run_well_defined_def by simp
    hence "pre_omega_word omega_word N \<in> language_auto \<A>" unfolding language_auto_def by blast
    hence "\<exists> N > n . pre_omega_word omega_word N \<in> language_auto \<A>" using N by blast
  }
  hence prefixes_in: "\<forall> n . \<exists> N > n . pre_omega_word omega_word N \<in> language_auto \<A>" by auto
  have inf_indices: "infinite {N . pre_omega_word omega_word N \<in> language_auto \<A>}"
  proof(rule ccontr)
    assume "\<not> infinite {N. pre_omega_word omega_word N \<in> language_auto \<A>}"
    hence "finite {N. pre_omega_word omega_word N \<in> language_auto \<A>}" by simp
    hence "\<exists> m . \<forall> N \<in> {N . pre_omega_word omega_word N \<in> language_auto \<A>} . N \<le> m" using finite_nat_set_iff_bounded_le by blast
    then obtain m where m: "\<forall> N \<in> {N . pre_omega_word omega_word N \<in> language_auto \<A>} . N \<le> m" by blast
    obtain N where "N > m \<and> pre_omega_word omega_word N \<in> language_auto \<A>" using prefixes_in by blast
    thus False using m by force
  qed
  have "inj_on (\<lambda>N. pre_omega_word omega_word N) {N . pre_omega_word omega_word N \<in> language_auto \<A>}" using pre_omega_word_length inj_onI by metis
  hence inf: "infinite ((\<lambda>N . pre_omega_word omega_word N) ` {N . pre_omega_word omega_word N \<in> language_auto \<A>})" using inf_indices by (simp add: finite_image_iff)
  have "((\<lambda>N . pre_omega_word omega_word N) ` {N . pre_omega_word omega_word N \<in> language_auto \<A>}) \<subseteq> language_auto \<A>" by auto
  thus ?thesis using inf finite_subset by blast
qed

theorem nonempty_omega_language_implies_infinite_finite_language:
  assumes "omega_language_auto \<A> \<noteq> {}"
  shows "infinite (language_auto \<A>)"
  using assms accepted_omega_word_implies_infinite_finite_language by blast

corollary finite_finite_language_implies_empty_omega_language:
  assumes "finite (language_auto \<A>)"
  shows "omega_language_auto \<A> = {}"
  using assms nonempty_omega_language_implies_infinite_finite_language by blast

definition \<A>_finite_inf_empty_omega :: "'s state \<Rightarrow> 's state \<Rightarrow> 'a symbol \<Rightarrow> ('s, 'a) automaton" where "\<A>_finite_inf_empty_omega s1 s2 a = Automaton {s1, s2} {a} {(s1, a, s1), (s1, a, s2)} s1 {s2}"

definition run_to_acc :: "'s state \<Rightarrow> 's state \<Rightarrow> nat \<Rightarrow> 's run" where "run_to_acc s1 s2 n = replicate n s1 @ [s2]"

lemma well_def_\<A>_finite_inf_empty_omega:
  assumes "s1 \<noteq> s2"
  shows "auto_well_defined (\<A>_finite_inf_empty_omega s1 s2 a)"
  unfolding \<A>_finite_inf_empty_omega_def auto_well_defined_def by auto

lemma pump_singleton_nth:
  assumes "i < n"
  shows "pump [a] n ! i = a"
  using assms by (induction n arbitrary: i) (auto simp: nth_Cons split: nat.splits)

lemma run_to_acc_accepts:
  assumes "s1 \<noteq> s2"
  shows "run_accepting (run_to_acc s1 s2 (Suc n)) (\<A>_finite_inf_empty_omega s1 s2 a) (pump [a] (Suc n))"
proof -
  let ?A = "\<A>_finite_inf_empty_omega s1 s2 a"
  have len: "length (run_to_acc s1 s2 (Suc n)) = length (pump [a] (Suc n)) + 1" unfolding run_to_acc_def by (induction n) auto
  have init: "run_to_acc s1 s2 (Suc n) ! 0 = initial_state ?A \<and> initial_state ?A \<in> states ?A" unfolding run_to_acc_def \<A>_finite_inf_empty_omega_def by auto
  {
    fix i assume i: "i < length (run_to_acc s1 s2 (Suc n)) - 1"
    hence i_le: "i \<le> n" unfolding run_to_acc_def by auto
    hence word_i: "pump [a] (Suc n) ! i = a" using pump_singleton_nth[of i "Suc n" a] by auto
    consider (before) "i < n" | (last) "i = n" using i_le by linarith
    hence "(run_to_acc s1 s2 (Suc n) ! i, pump [a] (Suc n) ! i, run_to_acc s1 s2 (Suc n) ! (i + 1)) \<in> transitions ?A"
    proof cases
      case before
      hence r_i: "run_to_acc s1 s2 (Suc n) ! i = s1" unfolding run_to_acc_def  by (metis i_le le_imp_less_Suc length_replicate nth_append nth_replicate)
      have "run_to_acc s1 s2 (Suc n) ! (i + 1) = s1" using before unfolding run_to_acc_def  by (metis Suc_eq_plus1 length_replicate list_properties_length not_less_eq nth_replicate)
      thus ?thesis using r_i word_i unfolding \<A>_finite_inf_empty_omega_def by auto
    next
      case last
      hence r_i: "run_to_acc s1 s2 (Suc n) ! i = s1" unfolding run_to_acc_def by (metis length_replicate lessI nth_append nth_replicate)
      have "run_to_acc s1 s2 (Suc n) ! (i + 1) = s2" using last unfolding run_to_acc_def by (metis Suc_eq_plus1 length_replicate nth_append_length)
      thus ?thesis using r_i word_i unfolding \<A>_finite_inf_empty_omega_def by auto
    qed
  }
  hence "\<forall> i < length (run_to_acc s1 s2 (Suc n)) - 1 . (run_to_acc s1 s2 (Suc n) ! i, pump [a] (Suc n) ! i, run_to_acc s1 s2 (Suc n) ! (i + 1)) \<in> transitions ?A" by blast
  hence run_well: "run_well_defined (run_to_acc s1 s2 (Suc n)) ?A (pump [a] (Suc n))" using len init unfolding run_well_defined_def pseudo_run_well_defined_def by blast
  have "last (run_to_acc s1 s2 (Suc n)) \<in> accepting_states ?A" unfolding run_to_acc_def \<A>_finite_inf_empty_omega_def by auto
  thus ?thesis using run_well unfolding run_accepting_def by blast
qed

lemma finite_language_infinite_example:
  assumes "s1 \<noteq> s2"
  shows "infinite (language_auto (\<A>_finite_inf_empty_omega s1 s2 a))"
proof -
  let ?A = "\<A>_finite_inf_empty_omega s1 s2 a"
  have subset: "{pump [a] (Suc n) | n . True} \<subseteq> language_auto ?A" using run_to_acc_accepts[OF assms] unfolding language_auto_def by fast
  have "inj_on (\<lambda>n . pump [a] (Suc n)) UNIV"
  proof
    fix m n assume "pump [a] (Suc m) = pump [a] (Suc n)"
    hence "length (pump [a] (Suc m)) = length (pump [a] (Suc n))" by simp
    thus "m = n" by (induction m arbitrary: n) (case_tac n, auto)+
  qed
  hence "infinite {pump [a] (Suc n) | n . True}" using infinite_UNIV_nat by (simp add: finite_image_iff full_SetCompr_eq)
  thus ?thesis using subset finite_subset by blast
qed

lemma omega_language_empty_example:
  assumes "s1 \<noteq> s2"
  shows "omega_language_auto (\<A>_finite_inf_empty_omega s1 s2 a) = {}"
proof(rule ccontr)
  assume "omega_language_auto (\<A>_finite_inf_empty_omega s1 s2 a) \<noteq> {}"
  then obtain x r where r: "omega_run_accepting r (\<A>_finite_inf_empty_omega s1 s2 a) x" unfolding omega_language_auto_def by blast
  hence trans: "\<forall> n . (r n, x n, r (n + 1)) \<in> transitions (\<A>_finite_inf_empty_omega s1 s2 a)" unfolding omega_run_accepting_def omega_run_well_defined_def omega_pseudo_run_well_defined_def by blast
  have "\<forall> n . \<exists> N > n . r N \<in> accepting_states (\<A>_finite_inf_empty_omega s1 s2 a)" using r unfolding omega_run_accepting_def by blast
  then obtain N where "r N \<in> accepting_states (\<A>_finite_inf_empty_omega s1 s2 a)" by blast
  hence s2: "r N = s2" unfolding \<A>_finite_inf_empty_omega_def by auto
  have "(r N, x N, r (N + 1)) \<in> transitions (\<A>_finite_inf_empty_omega s1 s2 a)" using trans by blast
  thus False using assms s2 unfolding \<A>_finite_inf_empty_omega_def by auto
qed

theorem converse_finite_language_empty_omega_false:
  assumes "infinite (UNIV :: 's set)" "infinite (UNIV :: 'a set)"
  shows "\<exists> \<A> :: ('s, 'a) automaton . auto_well_defined \<A> \<and> infinite (language_auto \<A>) \<and> omega_language_auto \<A> = {}"
proof -
  obtain s1 :: 's where s1: "s1 \<in> UNIV" by blast
  then obtain s2 :: 's where s2: "s2 \<in> UNIV \<and> s1 \<noteq> s2" using assms(1)  infinite_imp_nonempty infinite_remove  by (metis Diff_eq_empty_iff singletonI subsetI)
  obtain a :: 'a where a: "a \<in> UNIV" by blast
  let ?A = "\<A>_finite_inf_empty_omega s1 s2 a"
  have well: "auto_well_defined ?A" using well_def_\<A>_finite_inf_empty_omega s2 by fast
  have inf: "infinite (language_auto ?A)" using finite_language_infinite_example s2 by metis
  have "omega_language_auto ?A = {}" using omega_language_empty_example s2 by metis
  thus ?thesis using well inf by blast
qed
