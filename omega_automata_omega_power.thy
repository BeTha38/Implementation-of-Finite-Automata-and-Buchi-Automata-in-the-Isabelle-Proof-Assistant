...
proof -
  ...
  hence ex: "\<exists> n . i < return_index_start inf_word_list (Suc n)" by blast
  let ?n = "LEAST n . (\<lambda>n . i < return_index_start inf_word_list (Suc n)) n"
  have high: "i < return_index_start inf_word_list (Suc ?n)" using ex LeastI_ex by metis
  have low: "return_index_start inf_word_list ?n \<le> i"
  proof (cases "?n = 0")
    case True thus ?thesis unfolding return_index_start_def by simp
  next
    case False thus ?thesis using Least_le[of "\<lambda>n . i < return_index_start inf_word_list (Suc n)" "?n - 1"] by force
  qed
  {
